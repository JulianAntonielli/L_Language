...
class String
	...
	def parse line_number
		...
		raise VariableMissingInInstructionError.new("Missing variable in instruction at line #{line_number}") if variable.nil?
		...
		if instruction.class == GoToInstruction
			index += 1
			line = line[index..-1]
			instruction.label_2 = extract_label
		end
		...
		each_char do |c|
			...
			when :letter
				if c.isDigit?
					...
					number = "1" if number == ""
					return variable = Variable.new(letter, number.to_i)
				end
			end
		end
	end
