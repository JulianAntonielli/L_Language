...
require_relative '../Instruction/increment_instruction'
require_relative '../Instruction/decrement_instruction'
require_relative '../Instruction/goto_instruction'
require_relative '../Label/label'
require_relative '../Variable/variable'
require_relative '../Utils/string'
  
module Parser
  class << self

    def extract_instructions_from_file(file)
      instructions = []
    ...
    def parse line
      ...
      if instruction.class == GoToInstruction
        line = line[1..-1]
        instruction.goto_label = extract_label line
      end

      instruction
      # Que hacer si es goto....x`
      #instruction.goto_label = extract_label(line, line_number) if instruction.class == GoToInstruction
    end
      ...
      line.each_char do |c|
        ...
        when :letter
          if c.digit?
            ...
            number = "1" if number == ""
            return variable = Variable.get(letter, number) || Variable.new(letter, number)
          end
        end
      end
    end
